...
def optimizeARIMA(parameters_list, d,  s):
    """
    ...
    for param in tqdm.notebook.tqdm(parameters_list):
        ...
        try:
            model = statsmodels.tsa.arima_model.ARIMA(rt,  order=(param[0], param[1], param[2])).fit()
...
result_table = optimizeARIMA(parameters_list, d,  s)

tr = pd.Series([float(train.replace.values[i]) for i in range(len(train.replace.values))])

rm = []
for i in range(result_table.shape[0]):
    arim = statsmodels.tsa.arima_model.ARIMA(tr,  order=(result_table['parameters'][i][0], result_table['parameters'][i][1], result_table['parameters'][i][2])).fit()
    p = arim.forecast(12)
    rm.append(rmse(test.Sales.values, p[0]))
...
arim = statsmodels.tsa.arima_model.ARIMA(tr,  order=(0, 1, 3)).fit()
arim.summary()
arima_output = pd.DataFrame(arim.forecast(12))
arim_out = pd.DataFrame(arima_output.iloc[0, :])
...
arim.resid.describe()
statsmodels.stats.diagnostic.acorr_ljungbox(arim.resid, 24)

# some of the p value is less then 0.05 that is autocorrelation is present. we can modify the arima more.

autocorrelation_plot(arim.resid)
sns.distplot(arim.resid)
